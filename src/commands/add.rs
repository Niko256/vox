...
impl AddCommand {
    ...
    /// Recursively finds the repository root by looking for .vcs directory
    fn find_repository_root() -> Result<PathBuf> {
        let mut current = env::current_dir()?;
        loop {
            if current.join(".vcs").is_dir() {
                return Ok(current);
            }
            if !current.pop() {
    ...
    fn load_or_create_index(repo_root: &Path) -> Result<Index> {
        ...
        let index_path = repo_root.join(".vcs/index");
    ...
    fn add_path(&mut self, path: &Path, relative_base: &Path) -> Result<()> {
        ...
        if absolute_path.is_file() {
            ...
            for entry in WalkDir::new(&absolute_path)
                ...
                .into_iter()
                .filter_entry(move |e| {
                    !e.path().starts_with(repo_root.join(".vcs"))
    ...
    fn save_index(&self) -> Result<()> {
        let index_path = self.repo_root.join(".vcs/index");
